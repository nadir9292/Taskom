...
const FROM_EMAIL = 'Taskom <noreply@taskom.app>'
...
export async function sendInviteEmail(to: string, teamName: string, token: string) {
  ...
  await sendEmail(
    to,
    `Invitation à rejoindre ${teamName} sur Taskom`,
    `
...
export async function sendRegisterAndInviteEmail(to: string, teamName: string, token: string) {
  ...
  await sendEmail(
    to,
    `Rejoins ${teamName} sur Taskom`,
    `
      <div style="font-family: sans-serif; max-width: 480px; margin: auto;">
        <h2>Tu as été invité à rejoindre <strong>${teamName}</strong> sur Taskom</h2>
